export default class UniqueArray<T> extends Array {
    ...
    // Filter
    // filter(callbackfn: (value: T, index: number, array: T[]) => unknown, thisArg?: any): UniqueArray<T> {
    //     var newIds = [...this.ids];
    //     var newArray = super.filter((_, index) => {
    //         var result = callbackfn(callbackfn.arguments[0], callbackfn.arguments[1], callbackfn.arguments[2]);
    //         if (result) newIds = newIds.filter((_, idIndex) => idIndex !== index);
    //         return result;
    //     }, thisArg);
    //     var newUniqueArray = new UniqueArray<T>(newArray);
    //     newUniqueArray._ids = newIds;
